# architecture.md — Template & Writing Rules

**Owns:** how the system is shaped — components, data model, data flow, and the reasoning behind structural decisions. **Never contains:** specific library/vendor names (that's `Tech_stack.md`), feature requirements (`PRD.md`), or build sequencing (`Implementation_plan.md`). Describe the *shape* of the system in technology-agnostic terms first; name specific tech only inside ADRs where the decision itself is the point.
...
- **Decide security and auth architecture early, not last.** Retrofitting authentication/authorization onto a system architected without it in mind is a common, costly mistake — some infrastructure resources (e.g. an API gateway's authorizer type) cannot be changed in place once created and require full resource replacement, not just an addition, to fix later. Section 7 (Authentication & Authorization) and Section 11 (Security) should be filled in during initial architecture design, even at MVP scope, not deferred to "we'll add auth later."
- Every component in the diagram must trace to a requirement in `PRD.md` — don't design speculative components for hypothetical future scale unless the PRD's non-functional requirements actually call for it.
- Log an ADR any time a structural decision was genuinely debatable — not for trivial choices, but for anything a future reader might reasonably ask "why was it built this way instead of the obvious alternative?"
- New ADRs get appended, never rewritten in place — if a decision is later reversed, add a new ADR referencing and superseding the old one; don't erase the history.
