# PRD.md — Template & Writing Rules

**Owns:** what the product does, for whom, and why. **Never contains:** specific technologies, library names, database schema, code structure, build sequencing — those belong in other files. If you're naming a framework or a table column here, stop and move it.
...
- Every functional requirement should be traceable to a goal — if a requirement doesn't serve any stated goal, question whether it belongs.
- Write for a reader who knows nothing about the implementation — no framework names, no "the API will," no schema references.
- Flag every inferred/assumed detail explicitly: `**Assumption:** [stated assumption] — confirm or correct.`
