...
0. If `.buildkit/state.md` exists and the current feature is `awaiting approval`, do not implement until the user explicitly approves it.
1. Read `Progress.md` to determine current phase and next task(s).
...
6. Implement the task as the smallest coherent, independently testable slice. If `.buildkit/features/<slug>/spec.md` exists, use the approved feature spec and acceptance criteria as the immediate contract while the six root files remain authoritative for project facts.
...
8. Update `Progress.md`: move the task from "In Progress"/"Next Tasks" to "Completed," update "Current Phase" if the phase is now fully done, log any notable implementation-level decision under "Decisions Made," add a one-line Session Log entry, update "Last Updated."
